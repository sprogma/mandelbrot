...
& $CLANG (ls src/*.c -Exclude lli_test.c | % { 
...
& $CLANG src/lli_test.c -o test.exe ($IsLinux ? "-lm" : @()) ($CFLAGS|?{$_ -notmatch "-O\d"}) || $(Write-Host "Error: tests don't compiled" -Fore red; exit 1)
